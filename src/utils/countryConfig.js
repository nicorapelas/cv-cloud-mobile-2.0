// Country configurations for CV Cloud Mobile App
...
/**
 * Detect user's country from device locale/timezone (mobile approximation)
 * Returns 'ZA' as default for South Africa
 * @returns {string} Country code
 */
export const detectUserCountry = () => {
  // Try to get from Intl timezone (works in React Native)
  try {
    ...
    if (timezone.includes('Harare')) {
      return 'ZW'
    }
    if (timezone.includes('Gaborone')) {
      return 'BW'
    }
    if (timezone.includes('Windhoek')) {
      return 'NA'
    }
