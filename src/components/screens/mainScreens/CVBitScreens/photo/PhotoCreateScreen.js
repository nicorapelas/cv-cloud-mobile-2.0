...
const PhotoCreateScreen = () => {
    ...
    if (granted) {
      ...
      try {
        let data = await ImagePicker.launchImageLibraryAsync({
          ...
          allowsEditing: true, // Enable Expo's built-in cropping
          aspect: [1, 1], // Square crop (1:1 aspect ratio) - works on Android
  ...
  const pickFromCamera = async () => {
    ...
    if (granted) {
      ...
      try {
        ...
        console.error('Error taking/cropping image:', error)
        Alert.alert('Error', 'Failed to take or crop image. Please try again.')
      } finally {
        setCropping(false)
      }
    } else {
  ...
  const renderContent = () => {
    if (loading || cropping) return <LoaderFullScreen />
